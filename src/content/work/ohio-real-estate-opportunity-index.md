...
summary: A county-level real estate opportunity index combining demographic, housing, and market indicators to compare investment potential across all 88 Ohio counties.
...
Home price appreciation and the two growth indicators, population and income, carry the most weight because they were treated as the primary signals of market momentum in the index. Vacancy and affordability act as smaller corrective signals rather than primary drivers. The weights were set analytically rather than derived empirically, a choice discussed further under Limitations.

## Dashboard & County Exploration

The standardized, weighted scores were combined into a single Opportunity Index, used to rank all 88 counties, and then divided into four equal tiers: Very High, High, Moderate, and Low. The Power BI dashboard presents this through two views: a statewide overview and a county-level explorer.
...
The map makes the geographic pattern visible at a glance. Higher-scoring counties include Hardin, Union, Delaware, Fairfield, Licking, Perry, Geauga, Ashland, Highland, and Warren. Several are concentrated in central Ohio, while others are distributed across the state.
...
- The lowest-ranked group includes Wayne, Meigs, Belmont, Ottawa, and Noble.
...
The Opportunity Index is a structured way to compare counties across several housing-market signals at once; it is not a return forecast or a guarantee. A high-ranking county combines favorable growth and affordability signals in this snapshot, but a strong index score does not establish that a county is a good investment on its own. It flags a county worth investigating further alongside factors the index does not capture, including local zoning and permitting conditions, school quality, employer concentration, and property-level due diligence.

The current version reflects a single year, 2024, rather than a multi-year trend, so it cannot yet distinguish durable momentum from an unusually strong or weak year. The six indicator weights were also set analytically rather than derived empirically from the data. A sensitivity analysis testing how much the rankings change under different weighting schemes has not yet been run; this is a next step for the project. 
The underlying Python analysis notebook is not yet published alongside the dashboard, so the index construction is documented here and in the repository rather than independently reproducible from a public notebook at this time.
